...
cd uboot_todaro && make ${U_CFG} && make all && ./tools/mkimage -k ${CURPATH}/UBOOT_FIT_KEY -K ./arch/otto/dts/apro_03cvd_fit_control.dtb -e ${UIMAGE_ENTRY_POINT} -r -f ${ITS_FILE} ${FINAL_FIT_IMAGE} && make all
cd ../ && make all && cp uboot_todaro/${FINAL_FIT_IMAGE} release/uImage
mk_img_2k_alignment
# restore FIT image
cp release/uImage ${KERNEL_BUILD_FOLDER}/linux-4.4.x/arch/mips/boot/
# rebuild other image
