...
SECTION_MT
int mt_ram_simple_normal_test(u32_t addr, u32_t size)
{
    int i, j;
    u32_t start_value;
    volatile u32_t *read_start;
    volatile u32_t *start;
    const u32_t mp_dram_patterns[] = {0xa5a55a5a, 0x5a5aa5a5};

    printf("II: %s... ", __FUNCTION__);
    for (i=0; i < (sizeof(mp_dram_patterns)/sizeof(u32_t)); i++) {
        /* write pattern*/
        start = (u32_t *)(addr);
        read_start = (u32_t *)((u32_t)start+size-4);
        dcache_wr_inv_all();
        for (j=0; j<size; j=j+4) {
            *start = mp_dram_patterns[i];
            start++;
        }
        ...
        for (j=0; j<size; j=j+4) {
            start_value = (*read_start);
            if (start_value != mp_dram_patterns[i]) {
                EPRINTF("addr(0x%08x):0x%08x != pattern(0x%08x)\n",(u32_t)read_start,start_value,mp_dram_patterns[i]);                
                return MT_FAIL;
            }
            read_start--;
        }
...
SECTION_MT
int mt_ram_unaligned_wr(u32_t addr, u32_t size)
{
    u32_t wr_case;
    u32_t size_per_pat=size;
    int ret = MT_SUCCESS;
