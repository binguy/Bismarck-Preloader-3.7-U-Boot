...
static void
SHA512_Pad(crypto_sign_state *state, uint64_t tmp64[80 + 8])
{
    unsigned int r;
    unsigned int i;
    ...
    if (r < 112) {
        ...
        inline_memset(&state->buf[0], 0, 112);
    }
...
int
crypto_hash_sha512_init(crypto_sign_state *state)
{
    ...
    inline_memcpy(state->state, sha512_initial_state, sizeof sha512_initial_state);

    return 0;
}

int
crypto_hash_sha512_update(crypto_sign_state *state, const unsigned char *in,
...
int
crypto_hash_sha512_final(crypto_sign_state *state, unsigned char *out)
{
    uint64_t tmp64[80 + 8];
