...
MEMCNTLR_SECTION void __attribute__ ((weak)) mc_dump_pupd_en(mc_cntlr_opt_t *str) {
        printf("  pupd_en: %d\n", str->pupd_en);
        return;
}
MEMCNTLR_SECTION static inline void mc_xlat_n_assign_pupd_en(mc_dram_param_t * __attribute__ ((unused)), mc_cntlr_opt_t * __attribute__ ((unused)), mc_register_set_t *);
...
MEMCNTLR_SECTION static inline void mc_xlat_n_assign(void) {
	...
	if (meminfo.cntlr_opt->write_lvl_en != -1) {
        ...
        if (meminfo.cntlr_opt->pupd_en != -1) {
                mc_xlat_n_assign_pupd_en(dp, co, rs);
                if (ISTAT_GET(xlat) != MEM_XLAT_UNINIT) return;
        }
