...
static u32_t __DDR_Calibration_Full_Scan_Window(u32_t target_addr, u32_t len, u32_t resolution, u32_t w_start, u32_t r_start)
{
    ...
    printf("AK: Bit/max_r_s/max_r_l/max_w_s/max_w_l    Bit/max_r_s/max_r_l/max_w_s/max_w_l(Hex)\n");
    for(bit_loc = 0; bit_loc < 16; bit_loc++){
        printf("   [%2d]%8x%8x%8x%8x   [%2d]%8x%8x%8x%8x\n",
        bit_loc, max_r_seq_start_array[bit_loc], max_r_len_array[bit_loc], max_w_seq_start_array[bit_loc], max_w_len_array[bit_loc],
        (bit_loc+16), max_r_seq_start_array[(bit_loc+16)], max_r_len_array[(bit_loc+16)], max_w_seq_start_array[(bit_loc+16)], max_w_len_array[(bit_loc+16)]);
    }
    ...
    for(bit_loc = 0; bit_loc < 32; bit_loc++){
        ...
        if((max_w_len_array[bit_loc]<6) || (max_r_len_array[bit_loc]<6)){
            SYSTEM_RESET();
        }
    }
    if(PRODUZIONE){
