...
static int do_upimgtar(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	part_base = simple_strtoul(env_get("pfl_fdt1"), &endp, 16);
	part_sz = simple_strtoul(env_get("pfl_fdt1_sz"), &endp, 16);
...
END_DTB_UPDATE:
	...
	part_base = simple_strtoul(env_get("pfl_fdt2"), &endp, 16);
	part_sz = simple_strtoul(env_get("pfl_fdt2_sz"), &endp, 16);
	printf("DTB partition at 0x%08x, size=0x%08x\n", part_base, part_sz);
	#ifdef CONFIG_SPINOR_FLASH
	sprintf(cmd_buf, "sf erase %x +%x; sf write %x %x %x", part_base, part_sz, (unsigned long)(img + dtb_location), part_base, dtb_size);
	#else
...
#else
	part_base = simple_strtoul(env_get("pfl_kernel1"), &endp, 16);
	part_sz = simple_strtoul(env_get("pfl_kernel1_sz"), &endp, 16);
	printf("kernel partition at 0x%08x, size=0x%08x\n", part_base, part_sz);

	#ifdef CONFIG_SPINOR_FLASH
	sprintf(cmd_buf, "sf erase %x +%x; sf write %x %x %x", part_base, part_sz, (unsigned long)(img + kernel_location), part_base, kernel_size);
	#else
	sprintf(cmd_buf, "nand erase %x %x; nand write %x %x %x", part_base, part_sz, (unsigned long)(img + kernel_location), part_base, kernel_size);
	#endif
...
#else
		part_base = simple_strtoul(env_get("pfl_kernel2"), &endp, 16);
		part_sz = simple_strtoul(env_get("pfl_kernel2_sz"), &endp, 16);
		printf("kernel partition at 0x%08x, size=0x%08x\n", part_base, part_sz);
		#ifdef CONFIG_SPINOR_FLASH
		sprintf(cmd_buf, "sf erase %x +%x; sf write %x %x %x", part_base, part_sz, (unsigned long)(img + kernel_location), part_base, kernel_size);
		#else
...
#else
	part_base = simple_strtoul(env_get("pfl_rootfs1"), &endp, 16);
	part_sz = simple_strtoul(env_get("pfl_rootfs1_sz"), &endp, 16);
	printf("rootfs partition at 0x%08x, size=0x%08x\n", part_base, part_sz);
	#ifdef CONFIG_SPINOR_FLASH
	sprintf(cmd_buf, "sf erase %x +%x; sf write %x %x %x", part_base, part_sz, (unsigned long)(img + rootfs_location), part_base, rootfs_size);
	#else
	sprintf(cmd_buf, "nand erase %x %x; nand write %x %x %x", part_base, part_sz, (unsigned long)(img + rootfs_location), part_base, rootfs_size);
	#endif
...
#else
	part_base = simple_strtoul(env_get("pfl_rootfs2"), &endp, 16);
	part_sz = simple_strtoul(env_get("pfl_rootfs2_sz"), &endp, 16);
	printf("rootfs partition at 0x%08x, size=0x%08x\n", part_base, part_sz);
	#ifdef CONFIG_SPINOR_FLASH
	sprintf(cmd_buf, "sf erase %x +%x; sf write %x %x %x", part_base, part_sz, (unsigned long)(img + rootfs_location), part_base, rootfs_size);
	#else
...
END_ROOTFS2_UPDATE:
	...
	if (osgi_location && (!update_emmc_flag)) {
		...
		#ifdef CONFIG_SPINOR_FLASH
		sprintf(cmd_buf, "sf erase %x +%x; sf write %x %x %x", part_base, part_sz, (unsigned long)(img + osgi_location), part_base, osgi_size);
		#else
		sprintf(cmd_buf, "nand erase %x %x; nand write %x %x %x", part_base, part_sz, (unsigned long)(img + osgi_location), part_base, osgi_size);
		#endif
	...
	if (env_location && !update_emmc_flag) {
		if (do_flash_update_part("env", (unsigned long)(img + env_location), env_size)) {
			...
			ret = do_flash_update("pfl_bootenv1", "pfl_bootenv1_sz", (unsigned long)(img + env_location), env_size);
			if(ret){
		...
		if (do_flash_update_part("env2", (unsigned long)(img + env_location), env_size)) {
			// try use address and size to update
			ret = do_flash_update("pfl_bootenv2", "pfl_bootenv2_sz", (unsigned long)(img + env_location), env_size);
			if(ret){
